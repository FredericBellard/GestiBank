<?php
	...
	function getClient($id_client=0)
	{
		...
		if($id_user != 0)
		{
			$query .= " WHERE id_client=".$id__<?php
	header("Access-Control-Allow-Origin: *");
	header("Access-Control-Allow-Headers: access");
	header("Access-Control-Allow-Methods: DELETE");
	header("Content-Type: application/json; charset=UTF-8");
	...
	function updateUtilisateur($id_user)
	{
		...
		$password = $data["password"];
		$type_user = $data["type_user"];
		$query="UPDATE utilisateur SET nom='".$nom."', prenom='".$prenom."', adresse='".$adresse."', tel_user='".$tel_user."', email='".$email."', password='".$password."', type_user='".$type_user."' WHERE id=".$id;
		if(mysqli_query($conn, $query))
		{
			$response=array(
				'status' => 1,
